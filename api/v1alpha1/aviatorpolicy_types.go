...
type AviatorPolicySpec struct {
	...
	TargetRef        corev1.ObjectReference `json:"targetRef"`        // Reference to the Service
...
type AviatorPolicyStatus struct {
	...
	// Important: Run "make" to regenerate code after modifying this file
	LastCheckedTime metav1.Time `json:"lastCheckedTime,omitempty"` // Last time pods were probed
	CurrentLeader   string      `json:"currentLeader,omitempty"`   // Pod with the least latency
}
